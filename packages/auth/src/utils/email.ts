...
// Email HTML template for magic link
// TODO : update the image url with correct url
const getMagicLinkEmailHtml = (url: string) => `
...
const getPasswordResetEmailHtml = (url: string) => `
    <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif; max-width: 600px; margin: 0 auto; background-color: #1c1917; padding: 40px; border-radius: 12px; color: #f5f5f4;">
        ...
        <h2 style="color: #f5f5f4; text-align: center; font-size: 24px; font-weight: 600; margin-bottom: 24px;">Reset Your Password</h2>
        <p style="color: #d6d3d1; text-align: center; font-size: 16px; margin-bottom: 32px; line-height: 1.5;">We received a request to reset your password. Click the button below to set a new password. This link will expire in 15 minutes.</p>
        <div style="text-align: center;">
            <a href="${url}" style="display: inline-block; background-color: #b91c1c; color: #faf7f5; padding: 16px 32px; text-decoration: none; border-radius: 9999px; font-weight: 600; font-size: 16px; transition: opacity 0.2s;">Reset Password</a>
        </div>
        <p style="color: #d6d3d1; text-align: center; font-size: 14px; margin-top: 32px;">If you didn't request a password reset, you can safely ignore this email. Your password will remain unchanged.</p>
        <div style="border-top: 1px solid #44403c; margin-top: 32px; padding-top: 32px; text-align: center;">
            <p style="color: #d6d3d1; font-size: 12px;">© ${new Date().getFullYear()} TweetBatch. All rights reserved.</p>
        </div>
    </div>
...
export async function sendMagicLinkEmail(
  ...
  console.log(
    "[sendMagicLinkEmail] Runtime:",
    isWorkers ? "Cloudflare Workers (production)" : "Node.js/Wrangler dev",
  );

  if (isWorkers) {
    await sendWithWorkerMailer(
      email,
      url,
      gmailUser,
      gmailPassword,
      "Sign in to TweetBatch",
      getMagicLinkEmailHtml(url),
    );
  } else {
...
export async function sendPasswordResetEmail(
  ...
  console.log("[sendPasswordResetEmail] Password reset requested for:", email);
  console.log("[sendPasswordResetEmail] URL:", url);

  const isWorkers = isCloudflareWorkersProduction(gmailUser);
  console.log(
    "[sendPasswordResetEmail] Runtime:",
    isWorkers ? "Cloudflare Workers (production)" : "Node.js/Wrangler dev",
  );
  ...
  if (isWorkers) {
    await sendWithWorkerMailer(
      ...
      gmailUser,
      gmailPassword,
      "Reset Your Password - TweetBatch",
  ...
  console.log("[sendPasswordResetEmail] ✅ Password reset email processed");
}
