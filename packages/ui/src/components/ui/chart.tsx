            ...
            dangerouslySetInnerHTML={{
                __html: Object.entries(config)
                    .map(([key, config]) => {
                        ...
                        return color ? `[data-chart="${key}"] { --color-${key}: ${color}; }` : null
                    })
                    .join("\n"),
            }}
        />
    ...
    (
        ...
        return (
            <div
                ref={ref}
                className={cn(
                    "grid min-w-[8rem] items-start gap-1.5 rounded-lg border border-border/50 bg-background px-2.5 py-1.5 text-xs shadow-xl backdrop-blur-md",
                    className
                )}
            >
